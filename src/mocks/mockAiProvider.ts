// ============================================================
// MODULE: mocks/mockAiProvider.ts
// PURPOSE: Mock AI responses for Phase 1 development
// PHASE: 1
// STATUS: ACTIVE
// SWAP: Replace with real Claude SDK calls when API keys ready
// ============================================================
...
// ─── FINANCE SCRIPT MOCK RESPONSES ──────────────────────────

const MOCK_SCRIPT_BODIES: string[] = [
...
export async function mockAiCall(request: MockAiRequest): Promise<MockAiResponse> {
  ...
  log.info('Mock AI call completed', { latencyMs, promptLength: request.prompt.length });

  return {
    content: request.prompt, // caller will override with specific mock
    tokensUsed: Math.floor(request.prompt.length / 4) + Math.floor(Math.random() * 200),
    model: 'mock-claude-sonnet',
    latencyMs,
  };
}

export function getMockHooks(pattern: string, count: number): string[] {
  const hooks = MOCK_HOOKS[pattern] || MOCK_HOOKS['curiosity_gap'];
  const shuffled = [...hooks].sort(() => Math.random() - 0.5);
  return shuffled.slice(0, count);
}

export function getMockScriptBody(): string {
  const index = Math.floor(Math.random() * MOCK_SCRIPT_BODIES.length);
  return MOCK_SCRIPT_BODIES[index];
}
