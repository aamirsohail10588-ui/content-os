...
const { generateHooks } = require('./hookEngine');
...
const log = createLogger('VariantGenerator');

// ─── QUALITY SCORING ────────────────────────────────────────

function scoreVariant(hook, script, videoResult) {
  const scores = {};
  ...
  // Higher hook score + better pacing = higher retention estimate
...
async function generateVariant(topic, config, variantIndex) {
  ...
  try {
    ...
    const hook = hookResult.hooks[0]; // already sorted by score

    // Track cost
    costController.recordCost(variantId, config.aiProvider || 'claude', {
      inputTokens: 500, outputTokens: hookResult.tokensUsed || 200,
    });
    ...
    costController.recordCost(variantId, config.aiProvider || 'claude', {
      inputTokens: 800, outputTokens: scriptResult.tokensUsed || 500,
    });
    ...
    // Assemble video
    const videoResult = await assembleVideo(scriptResult.script, config.format || 'youtube_short');
    ...
    log.error('Variant generation failed', { variantId, variantIndex, error: err.message || String(err) });
    return null;
  }
}
...
async function generateVariants(topic, config) {
  ...
  if (!budget.canProceed) {
    log.error('Budget exceeded, cannot generate variants', { budget });
    return {
      success: false,
...
module.exports = { generateVariants, generateVariant, scoreVariant };
